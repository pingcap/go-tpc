...
func (w *Workloader) checkCondition1(ctx context.Context, warehouse int) error {
	...
	// 	W_YTD = sum(D_YTD)
	var diff float64
	query := "SELECT sum(d_ytd) - max(w_ytd) diff FROM district, warehouse WHERE d_pk BETWEEN ? AND ? AND w_id = ? group by d_w_id"

	rows, err := s.Conn.QueryContext(ctx, query, getDPK(warehouse, 0), getDPK(warehouse+1, 0), warehouse)
	if err != nil {
...
func (w *Workloader) checkCondition2(ctx context.Context, warehouse int) error {
	...
	var diff float64
	query := "SELECT POWER((d_next_o_id -1 - mo), 2) + POWER((d_next_o_id -1 - mno), 2) diff FROM district dis, (SELECT o_d_id,max(o_id) mo FROM orders WHERE o_pk BETWEEN ? AND ? GROUP BY o_d_id) q, (select no_d_id,max(no_o_id) mno from new_order where no_pk BETWEEN ? AND ? group by no_d_id) no where d_pk BETWEEN ? AND ? and q.o_d_id=dis.d_id and no.no_d_id=dis.d_id"

	rows, err := s.Conn.QueryContext(ctx, query,
		getOPK(warehouse, 0, 0), getOPK(warehouse+1, 0, 0),
		getNOPK(warehouse, 0, 0), getNOPK(warehouse+1, 0, 0),
		getDPK(warehouse, 0), getDPK(warehouse+1, 0))
	if err != nil {
		return fmt.Errorf("Exec %s failed %v", query, err)
	}
	defer rows.Close()
...
func (w *Workloader) checkCondition3(ctx context.Context, warehouse int) error {
	...
	query := "SELECT max(no_o_id)-min(no_o_id)+1 - count(*) diff from new_order where no_pk BETWEEN ? AND ? group by no_d_id"

	rows, err := s.Conn.QueryContext(ctx, query, getNOPK(warehouse, 0, 0), getNOPK(warehouse+1, 0, 0))
	if err != nil {
		return fmt.Errorf("Exec %s failed %v", query, err)
	}
	defer rows.Close()
...
func (w *Workloader) checkCondition4(ctx context.Context, warehouse int) error {
	...
	query := "SELECT count(*) FROM (SELECT o_d_id, SUM(o_ol_cnt) sm1, MAX(cn) as cn FROM orders,(SELECT ol_d_id, COUNT(*) cn FROM order_line WHERE ol_pk BETWEEN ? AND ? GROUP BY ol_d_id) ol WHERE o_pk BETWEEN ? AND ? AND ol_d_id=o_d_id GROUP BY o_d_id) t1 WHERE sm1<>cn"
...
func (w *Workloader) checkCondition5(ctx context.Context, warehouse int) error {
	...
	query := "SELECT count(*)  FROM orders LEFT JOIN new_order ON (no_w_id=o_w_id AND o_d_id=no_d_id AND o_id=no_o_id) where o_pk BETWEEN ? AND ? and ((o_carrier_id IS NULL and no_o_id IS  NULL) OR (o_carrier_id IS NOT NULL and no_o_id IS NOT NULL  )) "

	rows, err := s.Conn.QueryContext(ctx, query, getOPK(warehouse, 0, 0), getOPK(warehouse+1, 0, 0))
	if err != nil {
		return fmt.Errorf("Exec %s failed %v", query, err)
	}
	defer rows.Close()
...
(SELECT o_ol_cnt, order_line_count FROM orders
	LEFT JOIN (SELECT ol_w_id, ol_d_id, ol_o_id, count(*) order_line_count FROM order_line WHERE ol_pk BETWEEN ? AND ? GROUP BY ol_w_id, ol_d_id, ol_o_id ORDER by ol_w_id, ol_d_id, ol_o_id) AS order_line
	ON orders.o_d_id = order_line.ol_d_id AND orders.o_id = order_line.ol_o_id
	WHERE orders.o_pk BETWEEN ? AND ?) AS T
...
func (w *Workloader) checkCondition7(ctx context.Context, warehouse int) error {
	...
	query := "SELECT count(*) FROM orders, order_line WHERE o_pk BETWEEN ? AND ? AND ol_pk BETWEEN ? AND ? AND o_id = ol_o_id AND o_d_id=ol_d_id AND ((ol_delivery_d IS NULL and o_carrier_id IS NOT NULL) or (o_carrier_id IS NULL and ol_delivery_d IS NOT NULL ))"
...
func (w *Workloader) checkCondition9(ctx context.Context, warehouse int) error {
	...
	query := "SELECT COUNT(*) FROM (select d_id,d_w_id,sum(d_ytd) s1 from district WHERE d_pk BETWEEN ? AND ? group by d_id,d_w_id) d,(select h_d_id,h_w_id,sum(h_amount) s2 from history WHERE  h_w_id = ? group by h_d_id, h_w_id) h WHERE h_d_id=d_id AND d_w_id=h_w_id and s1<>s2"
...
SELECT count(*) FROM
	(SELECT * FROM
		(SELECT o_d_id, count(*) order_count FROM orders WHERE o_pk BETWEEN ? AND ? GROUP BY o_d_id) orders
        JOIN (SELECT no_d_id, count(*) new_order_count FROM new_order WHERE no_pk BETWEEN ? AND ? GROUP BY no_d_id) new_order
        ON orders.o_d_id = new_order.no_d_id
	) order_new_order
JOIN (SELECT c_d_id, count(*) customer_count FROM customer WHERE c_pk BETWEEN ? AND ? GROUP BY c_d_id) customer
ON order_new_order.no_d_id = customer.c_d_id
WHERE order_count - 2100 != new_order_count`
...
func (w *Workloader) checkCondition12(ctx context.Context, warehouse int) error {
	...
	query := `SELECT count(*) FROM (SELECT  c.c_id, c.c_d_id, c.c_balance c1, c_ytd_payment, 
		(SELECT sum(ol_amount) FROM orders STRAIGHT_JOIN order_line 
		WHERE o_pk BETWEEN ? AND ? AND OL_D_ID = O_D_ID AND OL_O_ID = O_ID AND OL_DELIVERY_D IS NOT NULL AND 
		ol_pk BETWEEN ? AND ? AND O_D_ID=c.C_D_ID AND O_C_ID=c.C_ID) sm FROM customer c WHERE  c.c_pk BETWEEN ? AND ?) t1 
