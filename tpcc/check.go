...
func (w *Workloader) checkCondition2(ctx context.Context, warehouse int) error {
	...
	var diff float64
	query := "SELECT POWER((o_nexi_o_id -1 - max(o_id)), 2) + POWER((o_nexi_o_id -1 - max(no_o_id)), 2) FROM district, warehouse WHERE d_w_id = o_w_id AND d_w_id = no_w_id AND d_id = o_d_id AND d_id = no_d_id AND w_id = ? group by d_w_id"
